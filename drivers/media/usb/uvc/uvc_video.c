...
static void uvc_read_stream_error(struct work_struct *work)
{
	...
	__u8 err_code;
	struct uvc_streaming *stream;
	struct my_work_struct* mwrk = container_of(work, struct my_work_struct, work);

	stream = mwrk->stream;
	ret = __uvc_query_ctrl(stream->dev, UVC_GET_INFO, 0, stream->intfnum,
		UVC_VS_STREAM_ERROR_CODE_CONTROL, &err_code,
		1, uvc_timeout_param);

	printk(KERN_ERR "--->bytes = %d: code = %d\n", ret, (int)err_code);

}

static struct workqueue_struct *err_wq;
struct my_work_struct my_wrk;
/* ------------------------------------------------------------------------
 * Video codecs
 */
		...
		struct uvc_buffer *buf, const __u8 *data, int len)
{
	...
	if (data[1] & UVC_STREAM_ERR) {
		my_wrk.stream = stream;
		queue_work(err_wq, &my_wrk.work);
		printk(KERN_ERR "---> err\n");
...
int uvc_video_init(struct uvc_streaming *stream)
{
	...
	if (err_wq == NULL) {
		err_wq = create_workqueue("error_rd_q");
		INIT_WORK(&my_wrk.work, uvc_read_stream_error);
	}
